...
exports.run = (client, message, args) => {
 ...
 if (!message.member.permissions.has("MANAGE_CHANNELS")) return message.channel.send(`${red} **Sorry, you do not have permission to perform the lockdown command.** `);
  ...
  if (validUnlocks.includes(time)) {
    message.channel.overwritePermissions(message.guild.id, {
      ...
      message.channel.send(`${verified} **Lockdown lifted.**`);
    ...
    message.channel.overwritePermissions(message.guild.id, {
      ...
      message.channel.send(`${green} **Channel locked down for ${ms(ms(time), { long:true })}.**`).then(() => {
        ...
        client.lockit[message.channel.id] = setTimeout(() => {
          message.channel.overwritePermissions(message.guild.id, {
            SEND_MESSAGES: true
          }).then(message.channel.send(`${verified} **Lockdown lifted.**`))
          delete client.lockit[message.channel.id];
        }, ms(time));
      }).catch(error => {
        console.log(error);
