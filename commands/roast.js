...
exports.run = async (client, message, args) => {
  ...
  let green = client.guilds.get("608289298537775127").emojis.find(emoji => emoji.name === "green");
    ...
    if (message.mentions.users === message.author.username) return message.reply(`${red} **You can not roast yourself** `);
    if (message.mentions.users.size < 1) return message.reply(`${red} **You must mention someone to roast them.** `)
    var roast = [
    ...
    const embed = new Discord.RichEmbed()
        ...
        .setFooter(`Roasted by ${message.author.tag}`, message.author.avatarURL);
