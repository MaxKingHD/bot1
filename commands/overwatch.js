...
module.exports.run = async (client, message, args) => {
  ...
  if (!args[0])
    return message.channel.send(
      `${red} **Error. Please specify a username with a battleTag. <prefix>ow <username#battleTag> [platform pc/psn/xbl]** `
    );
  ...
  if (!result)
    return message.channel.send(
      `${red} **Error. Please provide your battle tag right after your username. ow <username#battleTag> [platform pc/psn/xbl]** `
    );
  ...
  if (!args[1])
    return message.channel.send(
      `${red} **Error. Please specify a platform pc/psn/xbl. ow <username#battleTag> [platform pc/psn/xbl]** `
    );
  ...
  if (platform == "wrong")
    return message.channel.send(
      `${red} **Error. Please specify a **valid** platform pc/psn/xbl. ow <username#battleTag> [platform pc/psn/xbl]** `
    );
  ...
  async function getProfileStats(platform, username) {
    ...
    let { body } = await superagent
      ...
      .on("error", err => {
        //console.error(err);
        return message.channel.send(
          `${red} **Error occurred while retrieving player stats. Please try again later. ow <username#battleTag> [platform pc/psn/xbl]** `
        );
      });
    ...
    if (body.private)
      return message.reply(
        `${red} **Blizzard is not letting me get the player stats. Player profile needs to be public to have access to stats. You can do it by going to Options > Social > Change profile to public in the game client.** `
      );
